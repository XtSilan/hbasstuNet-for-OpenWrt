...
- Portal 请求绑定所选网络的源 IPv4，并按该源地址查对应接口路由表；以 IPv4/MAC/SessionId 校验服务端身份，避免同网段双 WAN 或 mwan3 让认证请求串线
- 校验服务端返回的账号、IPv4、MAC 和 SessionId，拒绝把其他账户的会话误判为成功
- 由 procd 管理后台服务并支持异常拉起
- 账号配置保存在独立 UCI 配置文件中

> [!NOTE]
> “校园网接口”应选择实际连接校园网的 OpenWrt 逻辑网络，通常是 `wan`；无线中继或自定义网络也可能叫 `wwan` 或其他名称。插件会动态解析该网络的三层设备、IPv4、MAC、网关及可用策略路由表，不应选择只服务于内网设备的 `lan`。
...
3. 将 Portal 请求绑定到该网络的源 IPv4，并为该源地址选择对应接口的路由表
4. 校验 Portal 返回的账号、IPv4、MAC 和 SessionId，拒绝接管其他线路的会话
5. 在地址尚未获取时等待，而不是使用错误的出口反复认证

这个选项填写的是 OpenWrt 的**逻辑网络名**，不是 `eth0`、`wlan0` 之类的 Linux 设备名。配置页会从 OpenWrt 的 UCI 网络配置读取已定义的逻辑网络，因此无需手工输入。

如果该接口已经由 mwan3 管理，认证实例会复用 mwan3 为该接口创建的路由表；如果没有，则使用运行期私有表。插件只维护自身的源地址规则，不会修改 `/etc/config/network` 或 `/etc/config/mwan3`。接口、IPv4 或网关变化时会自动重建，进程退出时会精确清理。
...
| 网络信息 | ubus + jsonfilter | 获取逻辑网络状态、IPv4、设备名和网关 |
| 出口约束 | iproute2 策略路由 | 让每个认证实例的源 IPv4 固定使用对应接口路由表 |
